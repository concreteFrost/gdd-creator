...
function OverviewMechanicsElement({
  location,
  characters,
  t,
}: LocationElementProps) {
  const locationCharacters = characters.filter((character: Character) =>
    location.characters.find((id: string) => id === character.id)
  );
  return (
    <div>
      <section className={overviewStyles.sub_header}>
        ...
        {location.img ? (
          <LocationImage
            path={location.img ? location.img : ""}
            alt={"main image"}
            width="700px"
          ></LocationImage>
        ) : null}
        ...
        <div className={overviewStyles.paragraph_element}>
          ...
          <ul>
            {locationCharacters.length > 0 ? (
              locationCharacters.map((character: Character) => (
